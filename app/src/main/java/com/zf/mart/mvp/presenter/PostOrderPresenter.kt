...
class PostOrderPresenter : BasePresenter<PostOrderContract.View>(), PostOrderContract.Presenter {
    ...
    override fun requestPostOrder(act: Int,
                                  address_id: String,
        ...
        val disposable = model.requestPostOrder(act, address_id, invoice_title, taxpayer, invoice_desc, coupon_id, pay_points, user_money, user_note, pay_pwd, goods_id, goods_num, item_id, action, shop_id, take_time, consignee, mobile)
                .subscribe({
                    mRootView?.apply {
                        dismissLoading()
                        when (it.status) {
